 ...
 */
@Slf4j
public class YtTrackJob1 implements Job {
	@Autowired
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		trackQuery.eq(Needtotracknumber121::getErrorflag,0);
		...
		for(Needtotracknumber121 needtotracknumber121:list){
			//开始交运
			needtotracknumber121.setErrorflag(1);
			needtotracknumber121.setErrordescr("正在获取订单号...");
			...
			if(order0==null){
				needtotracknumber121.setErrorflag(98);
				needtotracknumber121.setErrordescr("订单不存在!");
				needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
				return;
			}
			//订单商品详情
			List<Orderitem0> orderItem0 = orderitem0Service.getItemsProduct(needtotracknumber121.getOrderid());
			if(orderItem0==null || orderItem0.size()<=0){
				needtotracknumber121.setErrorflag(98);
				needtotracknumber121.setErrordescr("订单商品详情不存在!");
				needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
				return;
			}
			...
			orderplus0Query.eq(Orderplus0::getOrderid,needtotracknumber121.getOrderid());
			Orderplus0 orderplus0 = orderplus0Service.getOne(orderplus0Query);
			if(orderplus0==null ){
				needtotracknumber121.setErrorflag(98);
				needtotracknumber121.setErrordescr("订单买家信息不存在!");
				needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
				return;
			}
			...
			findMyLogisticsChannel.setCompanyId(needtotracknumber121.getCompanyid());
			findMyLogisticsChannel.setMyLogisticsChannelId(needtotracknumber121.getMylogisticschannelid());
			...
			if (resultMylogisticschannel.getAck()!=1000 && resultMylogisticschannel.getData()!=null){
				...
				needtotracknumber121.setErrorflag(98);
				needtotracknumber121.setErrordescr("货运方式不存在!");
				needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
				return;
			}
			//物流渠道
			FindLogisticsChannel findLogisticsChannel = new FindLogisticsChannel();
			findLogisticsChannel.setLogisticsChannelId(needtotracknumber121.getLogisticschannelid());
			findLogisticsChannel.setLogisticsId(needtotracknumber121.getLogisticsid());
			...
			if (resultLogisticschannel.getAck()!=1000 && resultLogisticschannel.getData()!=null){
				...
				needtotracknumber121.setErrorflag(98);
				needtotracknumber121.setErrordescr("物流渠道不存在!");
				needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
				return;
			}
			...
			if(needtotracknumber121.getPropertyjson()!=null){
				propertyJson = JSONObject.parseObject(needtotracknumber121.getPropertyjson(), PropertyJson.class);
			}
			...
			if(requestJson!=null) {
				...
				if (resultDbMylogisticsaccount.getAck() != 1000 && resultDbMylogisticsaccount.getData() != null) {
					...
					needtotracknumber121.setErrorflag(98);
					needtotracknumber121.setErrordescr("物流账号不存在!");
					needtotracknumber121Service.saveOrUpdate(needtotracknumber121);
					return;
				}
			}
			...
			TrackResultModel trackResultModel = needtotracknumber121Service.runStep1(trackModel);
			needtotracknumber121.setErrorflag(trackResultModel.getFlag());
			needtotracknumber121.setErrordescr(trackResultModel.getDescr());
			if(trackResultModel.getFlag()==3){
				needtotracknumber121.setTracknumber(trackResultModel.getTrackNumber());
		...
		log.info(String.format(" 云途获取订单号step1 org.baifei.modules.job.yt.YtTrackJob1 ! 执行数:"+list.size()+" 时间:" + DateUtils.getTimestamp()));
	}
}
