 ...
 */
@Slf4j
public class QufahuoTrackJob1 implements Job {
	@Autowired
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		trackQuery.eq(Needtotracknumber114::getErrorflag,0);
		...
		for(Needtotracknumber114 needtotracknumber114:list){
			...
			findLogisticsChannel.setLogisticsChannelId(needtotracknumber114.getLogisticschannelid());
			findLogisticsChannel.setLogisticsId(needtotracknumber114.getLogisticsid());
			...
			if (resultLogisticschannel.getAck()!=1000 && resultLogisticschannel.getData()!=null){
				...
				needtotracknumber114.setErrorflag(98);
				needtotracknumber114.setErrordescr("物流渠道不存在!");
				needtotracknumber114Service.saveOrUpdate(needtotracknumber114);
				return;
			}
			...
			if(needtotracknumber114.getPropertyjson()!=null){
				propertyJson = JSONObject.parseObject(needtotracknumber114.getPropertyjson(), PropertyJson.class);
			}
			...
			TrackResultModel trackResultModel = needtotracknumber114Service.runStep1(trackModel);
		...
		log.info(String.format(" 去发货获取订单号step1 org.baifei.modules.job.qufahuo.QufahuoTrackJob1 ! 执行数:"+list.size()+" 时间:" + DateUtils.getTimestamp()));
	}
}
