 ...
 */
@Slf4j
public class FtLabelJob implements Job {
	@Autowired
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		trackQuery.eq(Needtotracknumber115::getErrorflag,3);
		trackQuery.eq(Needtotracknumber115::getIslabel,0);
		...
		for(Needtotracknumber115 needtotracknumber115:list){
            ...
            if(true){
				//开始交运
				needtotracknumber115.setIslabel(1);
				needtotracknumber115.setLabeldescr("开始下载单面...");
				...
				//交运完成
				TrackResultModel trackResultModel = needtotracknumber115Service.runStep2(trackModel);
				...
				if("3".equals(trackResultModel.getFlag())){
					...
					orderlabel.setCompanyId(needtotracknumber115.getCompanyid());
		...
		log.info(String.format(" 飞特下载订单 org.baifei.modules.job.ft.FtLabelJob  !  执行数:"+list.size()+" 时间:" + DateUtils.getTimestamp()));
	}
}
