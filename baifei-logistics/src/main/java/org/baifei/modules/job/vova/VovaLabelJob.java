 ...
 */
@Slf4j
public class VovaLabelJob implements Job {
	@Autowired
...
//			trackModel.setMylogisticsaccount(mylogisticsaccount);
			...
			TrackResultModel trackResultModel = needtotracknumber124Service.runStep3(trackModel);
		...
		log.info(String.format(" Vova下载订单 org.baifei.modules.job.vova.VovaLabelJob ! 执行数:"+list.size()+" 时间:" + DateUtils.getTimestamp()));
	}
}
